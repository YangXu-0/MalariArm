...
def vt(VTS_motor, end_height): #changes height of arm #modify a lot
    #global VTS_motor

    G_Geneva_gear_ratio = 50 # change this based on however Gerry designs the geneva gear
...
def let_go(RTS_motor, VTS_motor): #moves half, goes up
    #global RTS_motor, VTS_motor, pos

    # Assumes starts down
    pos = rotate(RTS_motor, 1, 2, half=True) # disconnect magnet
    vt(VTS_motor, 5)
    pos = rotate(RTS_motor, 1, 2, half=True) # reset back into the regular positions
    pos += 1 # (1/2 + 1/2) = 1
...
def testing(RTS_motor, VTS_motor, test_rts=False, test_vts=False):
    if test_rts == True:
        ...
        for i in range(200/6):
            ...
            RTS_motor[1].value = False
            time.sleep(0.009)
        RTS_motor[0].value = False
        for i in range(200/6):
            RTS_motor[1].value = True
            time.sleep(0.009)
            RTS_motor[1].value = False
            time.sleep(0.009)
            print(f"GOING FORWARD {RTS_motor[1].value}")
    '''
    while True:
        RTS_motor[1].value = False
        print("LOCK")
...
def testing_functions(RTS_motor, VTS_motor):
    vt(VTS_motor, height, 1)
    vt(VTS_motor, height, 5)

if __name__ == "__main__":
    start = True
    pos = 1
    height = 5
    RTS_motor = setup_motor(board.GP19, board.GP18) # dir step
    ...
    if start:
        ...
        led.value = True
        procedure(RTS_motor, VTS_motor, pos, height, start_time)
        #testing(RTS_motor, VTS_motor, test_rts=False,test_vts=True)
        #testing_functions(RTS_motor, VTS_motor)
        print("DONE!")
